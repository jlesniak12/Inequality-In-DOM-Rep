...
# ── H7: Hours × income scatter — NOMINAL scale, straight compliance lines ────
#
# Using nominal (non-log) income on the y-axis means:
#   - The hourly compliance boundary is a STRAIGHT LINE through the origin
#     with slope = MW_hourly × weeks_per_month
#   - The OT-adjusted boundary is two straight line segments that KINK
#     upward at 44hrs (slope × 1.35) and again at 68hrs (slope × 2.0)
#   - The non-compliant zone is visually as wide as the compliant zone
#     at any given hours level — no log-scale distortion
#
# Y-axis is capped at 4 × MW_monthly to keep the compliance region visible.
# Workers above the cap are noted in the caption but not plotted.
# They are all hourly-compliant so excluding them does not bias the picture.
#
# THREE reference lines:
#   Solid black  — monthly MW floor (horizontal): earns at least MW_monthly
#   Dashed black — hourly compliance boundary (linear through origin)
#   Dotted dark  — OT-adjusted boundary (kinked at 44 and 68 hrs)
#
# Colour = monthly compliance status.
# Workers in the blue band BELOW the dashed line: monthly-compliant but
# hourly non-compliant — the "hours extraction" population.
# Workers in orange above the dashed line: monthly non-compliant and also
# hourly non-compliant — straightforwardly underpaid.

cat("    H7: Hours x income scatter (nominal scale)...\n")
...
h7_data <- comp_formal %>%
  ...
  mutate(
    snap_label = factor(
      dplyr::recode(year_quarter, "2016Q2" = "2016 Q2", "2025Q2" = "2025 Q2"),
      levels = c("2016 Q2", "2025 Q2")
    ),
  ...
  group_by(snap_label) %>%
  slice_sample(n = 2000, weight_by = FACTOR_EXPANSION) %>%
  ungroup()

# Per-snapshot MW values (annual-average within the snapshot quarter)
mw_vals <- h7_data %>%
  group_by(snap_label) %>%
  summarise(
    mw_monthly = mean(real_minwage_harmonized, na.rm = TRUE),
    mw_hourly  = mean(real_minwage_harmonized / (23.83 * 8), na.rm = TRUE),
    .groups = "drop"
  )

# Y-axis cap: 4 × average MW monthly across both snapshots
y_cap <- 4 * mean(mw_vals$mw_monthly)

# Flag workers above cap (for caption note)
n_above_cap <- h7_data %>%
  filter(real_wage_compliance_primary > y_cap) %>%
  nrow()
pct_above_cap <- round(n_above_cap / nrow(h7_data) * 100, 1)

# Build reference lines — one set per snapshot
hours_seq <- seq(0, 90, by = 0.5)

ref_lines <- mw_vals %>%
  crossing(hours = hours_seq) %>%
  mutate(
    # Hourly compliance boundary: straight line, slope = MW_hourly × WEEKS_PM
    income_hourly = mw_hourly * hours * WEEKS_PM,
    
    # OT-adjusted boundary: kinks at 44 and 68 hrs
    #   ≤44hrs:  same as hourly (no OT yet)
    #   44-68hrs: OT tier 1 at 1.35× rate for excess hours
    #   >68hrs:  OT tier 2 at 2.0× rate for hours above 68
    income_ot = dplyr::case_when(
      hours <= 44 ~ mw_hourly * hours * WEEKS_PM,
      hours <= 68 ~ mw_hourly * (44 + (hours - 44) * 1.35) * WEEKS_PM,
      TRUE        ~ mw_hourly * (44 + 24 * 1.35 + (hours - 68) * 2.0) * WEEKS_PM
    )
  )

# Y-axis: DOP thousands for readability
scale_k <- 1000

fig_H7 <- ggplot(h7_data,
                 aes(x     = hours_worked_primary,
                     y     = real_wage_compliance_primary / scale_k,
                     colour = monthly_status)) +
  
  # Non-compliant zone shading (below hourly line)
  # Drawn as a ribbon so the zone is immediately obvious
  geom_ribbon(
    data        = ref_lines,
    aes(x = hours, ymin = 0, ymax = income_hourly / scale_k),
    inherit.aes = FALSE,
  ...
  # Monthly MW floor — horizontal line
  geom_hline(
    data        = mw_vals,
    aes(yintercept = mw_monthly / scale_k),
    colour      = "grey40", linetype = "dotted", linewidth = 0.55,
    inherit.aes = FALSE
  ) +
  
  # Hourly compliance boundary — straight line through origin
  geom_line(
    data        = ref_lines,
    aes(x = hours, y = income_hourly / scale_k),
    colour      = "black", linetype = "dashed", linewidth = 0.9,
    inherit.aes = FALSE
  ) +
  
  # OT-adjusted boundary — kinked line
  geom_line(
    data        = ref_lines,
    aes(x = hours, y = income_ot / scale_k),
    colour      = "#5B3A8E", linetype = "longdash", linewidth = 0.7,
    inherit.aes = FALSE
  ) +
  
  # 44hr and 68hr vertical guides
  geom_vline(xintercept = 44, linetype = "dotted", colour = "grey50",
             linewidth = 0.45) +
  geom_vline(xintercept = 68, linetype = "dotted", colour = "grey70",
             linewidth = 0.35) +
  
  # Labels for vertical guides
  annotate("text", x = 45, y = y_cap / scale_k * 0.97,
           label = "44 hrs\n(standard)", hjust = 0, vjust = 1,
           size = 2.4, colour = "grey40") +
  annotate("text", x = 69, y = y_cap / scale_k * 0.97,
           label = "68 hrs\n(OT tier 2)", hjust = 0, vjust = 1,
           size = 2.4, colour = "grey60") +
  
  facet_wrap(~snap_label) +
  
  scale_colour_manual(
    values = c("Monthly compliant"     = "#2C5F8A",
               "Monthly non-compliant" = "#C45C30"),
    name = "Monthly compliance status"
  ) +
  scale_x_continuous(breaks = c(0, 10, 20, 30, 44, 55, 68, 80),
                     limits = c(0, 90)) +
  scale_y_continuous(
    labels = function(x) paste0(x, "k"),
    limits = c(0, y_cap / scale_k)
  ) +
  
  labs(
    title    = "Hours Worked vs Monthly Income \u2014 Formal Workers",
    subtitle = paste0(
      "Black dashed = hourly MW boundary (straight line: income = MW_hourly \u00d7 hours \u00d7 weeks/month).\n",
      "Purple dashed = OT-adjusted boundary (kinks at 44 and 68 hrs for legal overtime premia).\n",
      "Orange shading = hourly non-compliance zone. Dotted grey = monthly MW floor."
    ),
    x       = "Weekly hours worked (primary job)",
    y       = "Monthly compliance income (real DOP thousands)",
    caption = paste(
      "Blue points below black dashed line: monthly-compliant but hourly non-compliant (hours extraction).",
      "Orange points below dashed line: non-compliant on both measures.",
      paste0(pct_above_cap, "% of sampled workers above y-axis cap (4\u00d7 MW) — all hourly-compliant, not shown."),
      "Formal private-sector employees. Government excluded. Random sample of 2,000 workers per snapshot.",
      SRC, sep = "\n"
    )
  ) +
  theme_surveytools(legend_position = "bottom") +
