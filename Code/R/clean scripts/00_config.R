...
config <- list(
  ...
  # --- File Paths (absolute, anchored to project root via {here}) --- #
  # here::here() resolves the repo root from a marker (.Rproj / .git) so these
  # are robust to R's working directory. "scripts" is the SINGLE place the
  # "clean scripts" folder name is written; 00_setup.R and every downstream
  # source() call compose off it, so a future rename touches only this line.
  paths = list(
