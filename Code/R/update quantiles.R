...
purrr::iwalk(
  plots_quantile,
  ~ ggsave(
    filename = file.path(save_path, "Income Inequality Trends", "Quantiles", paste(.y, save_type)),  # .y = name (spec_id)
...
q <- compute_quantiles(
  design      = design_indiv_q,
  var         = "real_independent_income_total",
  time_var    = "year_quarter",
  group_var   = NULL,
  probs       = fan_probs,
  subset_expr = quote(OCUPADO == 1 & real_independent_income_total > 0),
  return_se   = FALSE,
  return_ci   = FALSE
)

dplyr::count(q, time) %>% dplyr::arrange(desc(n))


qfan <- quantiles_to_fan(
  df = q,
  time_col = "time",
  group_col = "group",
  measure_col = "measure",
...
p_fan <- plot_ts_fan(
  ...
  time_var   = "time",
  center_col = "p50",
  bands      = list(c("p10","p90"), c("p25","p75")),
  series_col = "group",       # overlay groups if group_var was used in compute
  facet_col  = NULL,          # or facet_col="group" if you prefer panels
  title      = NULL,
  subtitle   = NULL,
  y_label    = NULL,
  x_label    = NULL
...
design_indiv_q$variables %>%
  dplyr::summarise(
    n = dplyr::n(),
    n_na = sum(is.na(year_quarter)),
    n_distinct = dplyr::n_distinct(year_quarter)
  )
